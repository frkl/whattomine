...
def patch_getwork(fname,patch_fname):
	...
	patched=0;
	for line in source:
		if patched==0 and line.find('using namespace std')>=0:
			target.write(line);
			#marker 1 : add patch
			for line_patch in patch:
				target.write(line_patch);
			patched=patched+1;
		elif patched==0 and line.find('unsigned int ParseConfirmTarget')>=0:
...
def install(coin,url,exec_name,rpcport,p2pport,nodes='',specify_algo='',getwork_patch='',configure_flags=''):
	#download
	subprocess.call('git clone %s ./%s'%(url,coin),shell=True);
	current_dir=os.getcwd();
	...
	elif getwork_patch=='btcv2':
		patch_getwork(os.path.join(current_dir,coin,'src','rpc','mining.cpp'),'getwork_patch_v3.cpp');
	...
	subprocess.call('cd %s/;./configure --with-incompatible-bdb %s'%(coin,configure_flags),shell=True);
...
#install('solaris','https://github.com/frkl/Solaris','solarisd',6201,8201);
...
#install('bitcore','https://github.com/LIMXTEC/BitCore','bitcored',6241,8241,'',getwork_patch='btcv2',configure_flags=' --enable-cxx --disable-shared --with-pic --prefix=$BDB_PREFIX CXXFLAGS="-fPIC" CPPFLAGS="-fPIC"');
...
#install('ravencoin','https://github.com/RavenProject/Ravencoin','ravend',6291,8291,'',getwork_patch='btcv2');
#northern ./configure --enable-cxx --disable-shared --with-pic --prefix=$BDB_PREFIX CXXFLAGS="-fPIC" CPPFLAGS="-fPIC"
install('infinex','https://github.com/frkl/Infinex','infinexd',6311,8321);
